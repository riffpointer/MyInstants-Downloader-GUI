...
def themed_icon(path: str, size: tuple[int, int]) -> customtkinter.CTkImage:
    ...
    return customtkinter.CTkImage(
        light_image=light_icon, dark_image=dark_icon, size=size
    )
...
class VirtualizedList(customtkinter.CTkFrame):
    def __init__(
        self,
        master,
        row_height: int = VIRTUAL_ROW_HEIGHT,
        row_gap: int = VIRTUAL_ROW_GAP,
        **kwargs,
    ):
        ...
        self.scrollbar = customtkinter.CTkScrollbar(
            self, orientation="vertical", command=self._on_scrollbar
        )
    ...
    def set_empty_widget(self, widget):
        ...
        self.empty_window_id = self.canvas.create_window(
        ...
        self.canvas.configure(
            scrollregion=(
                ...
                max(widget.winfo_reqheight() + 24, 100),
            )
        )
        self.canvas.itemconfigure(
            self.empty_window_id, width=max(self.canvas.winfo_width() - 20, 100)
        )
    ...
    def refresh_visible_rows(self):
        if self.empty_window_id is not None:
            ...
            self.canvas.configure(
                scrollregion=(
                    ...
                    0,
                    self.canvas.winfo_width(),
                    max(self.empty_widget.winfo_reqheight() + 24, 100),
                )
            )
            return
        ...
        start = max(int(top // self.total_row_height) - VIRTUAL_OVERSCAN, 0)
            ...
            int(bottom // self.total_row_height) + VIRTUAL_OVERSCAN + 1, len(self.items)
        )
        visible_keys = set()
...
class DownloadProgressWindow(customtkinter.CTkToplevel):
    def __init__(
        self, master, items, on_complete, download_dir: Path, on_next_page=None
    ):
        ...
        container = customtkinter.CTkFrame(
            self, corner_radius=16, fg_color=palette["dialog_bg"]
        )
        ...
        self.file_progress = customtkinter.CTkProgressBar(
            container, height=16, progress_color="#18b36f"
        )
        ...
        bytes_label = customtkinter.CTkLabel(
            file_meta,
        ...
        self.overall_progress = customtkinter.CTkProgressBar(
            container, height=16, progress_color="#2d8cff"
        )
        ...
        self.next_page_controls = customtkinter.CTkFrame(
    ...
    def retry_failed(self):
        ...
        self.cancel_current_button.configure(
            state="normal", text="Cancel Current", command=self.request_cancel_current
        )
        self.cancel_button.configure(
            state="normal", text="Cancel Queue", command=self.request_cancel
        )
        self.log(f"Retrying {len(self.items)} failed downloads.")
        self.start_download_thread()

    def update_progress(
        self, file_name, file_progress, file_bytes, speed, overall_fraction
    ):
    ...
    def finish_queue(self, message: str):
        ...
        self.cancel_button.configure(
            text="Close", state="normal", command=self.close_now
        )
        self.retry_button.pack_forget()
        self.next_page_controls.pack_forget()
            ...
            try:
                with open(target_path, "wb") as audio_file:
                    for chunk in response.iter_content(chunk_size=DOWNLOAD_CHUNK_SIZE):
                        ...
                        overall_fraction = (self.completed_files + file_progress) / max(
                            len(self.items), 1
                        )
                        self.safe_after(
                            self.update_progress,
        ...
        try:
            ...
            for item in self.items:
                if self.cancel_requested:
                    break
                self.safe_after(
                    self.status_var.set, "Downloading files from the current list..."
                )
                self.cancel_current_requested = False
                self.safe_after(
                ...
                try:
                    ...
                    if str(exc) == "Current download cancelled by user.":
                        ...
                        self.safe_after(
                            self.file_var.set, f"Skipped current file: {item['title']}"
                        )
            ...
            if self.cancel_requested:
                ...
                self.safe_after(
                    self.finish_queue,
                    "All files from the current page were downloaded.",
                )
        except Exception as exc:
...
class SettingsWindow(customtkinter.CTkToplevel):
    def __init__(self, app):
        ...
        self.appearance_var = tkinter.StringVar(
            value=app.settings.get("appearance_mode", "Dark")
        )
        self.hide_downloaded_var = tkinter.BooleanVar(
            value=app.settings.get("hide_downloaded", True)
        )
        self.server_choice_var = tkinter.StringVar(value=self.get_server_choice())
        self.server_custom_region_var = tkinter.StringVar(
            value=app.server_region if app.server_region not in {"us", "in"} else ""
        )
        ...
        folder_label = customtkinter.CTkLabel(
            folder_row, text="Downloads Folder", width=130, anchor="w"
        )
        folder_label.pack(side="left")

        folder_entry = customtkinter.CTkEntry(
            folder_row, textvariable=self.download_dir_var, width=340, height=36
        )
        ...
        appearance_label = customtkinter.CTkLabel(
            appearance_row, text="Theme", width=130, anchor="w"
        )
        ...
        filter_label = customtkinter.CTkLabel(
            filter_row, text="Hide Downloaded", width=130, anchor="w"
        )
        ...
        server_label = customtkinter.CTkLabel(
            server_row, text="Server", width=130, anchor="w"
        )
        ...
        self.custom_region_label = customtkinter.CTkLabel(
            custom_region_row, text="Custom Region", width=130, anchor="w"
        )
        ...
        base_url_label = customtkinter.CTkLabel(
            base_url_row, text="Base URL", width=130, anchor="w"
        )
    ...
    def update_server_fields(self):
        ...
        self.custom_region_label.configure(
            text_color=current_palette()["text_primary"]
            if is_custom
            else current_palette()["text_muted"]
        )

    def browse_folder(self):
        selected = filedialog.askdirectory(
            initialdir=self.download_dir_var.get() or os.getcwd()
        )
        if selected:
    ...
    def save(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Settings Error", f"Invalid server configuration: {exc}", parent=self
            )
            return
        if server_choice == "Custom" and not custom_region:
            messagebox.showerror(
                "Settings Error",
                "Enter a custom region code before saving.",
                parent=self,
            )
            return
        ...
        self.app_controller.apply_appearance_mode(
            self.appearance_var.get(), persist=False
        )
        self.app_controller.set_status(
            f"Downloads folder set to {selected_dir} | Server {normalized_region.upper()}"
        )
        self.close()
...
class InventoryWindow(customtkinter.CTkToplevel):
    ...
    def _build_empty_inventory(self):
        ...
        empty_icon = customtkinter.CTkLabel(
            empty, text="", image=self.app_controller.inventory_icon
        )
    ...
    def _finish_inventory_refresh(self, files, token: int):
        ...
        self.list_frame.set_items(
            files, self._create_inventory_row, item_key=lambda path: str(path)
        )
    ...
    def refresh_inventory(self):
        ...
        def _worker():
            try:
                files = sorted(
                    self.app_controller.download_dir.glob("*.mp3"),
                    key=lambda path: path.name.lower(),
                )
            except Exception as exc:
                self._safe_after(
                    self.summary_var.set, f"Unable to load inventory: {exc}"
                )
                return
    ...
    def rename_item(self, file_path: Path):
        ...
        if not cleaned_name:
            messagebox.showerror(
                "Rename Failed", "Please enter a valid file name.", parent=self
            )
            return
        ...
        if target_path.exists():
            messagebox.showerror(
                "Rename Failed", "A sound with that name already exists.", parent=self
            )
            return
        try:
            ...
            return
        self.app_controller.set_status(
            f"Renamed {file_path.name} to {target_path.name}"
        )
        self.app_controller.render_items(
            self.app_controller.current_items, self.app_controller.status_var.get()
        )
        self.refresh_inventory()
...
class MyInstantsApp:
    def __init__(self):
        ...
        self.server_region = normalize_region(
            self.settings.get("server_region", DEFAULT_REGION)
        )
        self.server_base_url = normalize_base_url(
            self.settings.get("server_base_url", DEFAULT_BASE_URL)
        )
        ...
        self.app.title(
            f"{APP_TITLE} | {self.status_var.get() if hasattr(self, 'status_var') else 'Starting'}"
        )
        self.app.winfo_toplevel().iconbitmap("resources/main.ico")
        self.app.configure(fg_color=current_palette()["app_bg"])

        self.download_icon = themed_icon("resources/download.png", (16, 16))
        self.hero_icon = themed_icon("resources/flush.png", (56, 56))
        self.folder_icon = themed_icon("resources/archive.png", (16, 16))
        self.left_icon = themed_icon("resources/arrow-left.png", (14, 14))
        self.right_icon = themed_icon("resources/arrow-right.png", (14, 14))
        self.search_icon = themed_icon("resources/search.png", (14, 14))
        self.search_empty_icon = themed_icon("resources/search.png", (42, 42))
        self.refresh_icon = themed_icon("resources/arrow-clockwise.png", (14, 14))
        self.play_icon = themed_icon("resources/play-fill.png", (14, 14))
        self.settings_icon = themed_icon("resources/gear-fill.png", (14, 14))
        self.home_icon = themed_icon("resources/house.png", (14, 14))
        self.inventory_icon = themed_icon("resources/box2-fill.png", (16, 16))
        self.rename_icon = themed_icon("resources/cursor-text.png", (16, 16))
        self.folder_open_icon = themed_icon("resources/folder2-open.png", (16, 16))
        self.delete_icon = themed_icon("resources/trash3.png", (16, 16))

        self.status_var = tkinter.StringVar(value="Loading page 1...")
        self.subtitle_var = tkinter.StringVar(
            value="Quickly download sounds from MyInstants!"
        )
    ...
    def build_menu(self):
        ...
        file_menu.add_command(
            label="Download All On Current View", command=self.download_all_current
        )
        file_menu.add_command(label="Inventory", command=self.open_inventory)
        file_menu.add_command(
            label="Open Downloads Folder", command=self.open_download_folder
        )
        ...
        navigate_menu.add_command(
            label="Back To Trending", command=lambda: self.load_page(1)
        )
        ...
        navigate_menu.add_separator()
        navigate_menu.add_command(
            label="Reload Current View", command=self.reload_current_view
        )
        ...
        appearance_menu.add_command(
            label="System Theme", command=lambda: self.apply_appearance_mode("System")
        )
        appearance_menu.add_command(
            label="Dark Theme", command=lambda: self.apply_appearance_mode("Dark")
        )
        appearance_menu.add_command(
            label="Light Theme", command=lambda: self.apply_appearance_mode("Light")
        )
    ...
    def build_layout(self):
        ...
        outer = customtkinter.CTkFrame(
            self.app, corner_radius=0, fg_color=palette["app_bg"]
        )
        outer.pack(fill="both", expand=True)

        content = customtkinter.CTkFrame(
            outer, corner_radius=0, fg_color=palette["app_bg"]
        )
        content.pack(fill="both", expand=True, padx=16, pady=16)

        toolbar = customtkinter.CTkFrame(
            content, corner_radius=14, fg_color=palette["panel_bg"]
        )
        ...
        list_panel = customtkinter.CTkFrame(
            content, corner_radius=16, fg_color=palette["panel_bg"]
        )
    ...
    def set_loading_state(
        self, status_message: str, results_message: str = "Loading..."
    ):
    ...
    def download_item(self, item):
        ...
        def _worker():
            try:
                ...
                if target_path.exists():
                    self.app.after(
                        0, self.set_status, f"Skipped existing file {target_path.name}"
                    )
                    return
    ...
    def rename_downloaded_item(self, file_path: Path):
        ...
        if not cleaned_name:
            messagebox.showerror(
                "Rename Failed", "Please enter a valid file name.", parent=self.app
            )
            return
        target_path = file_path.with_name(f"{cleaned_name}.mp3")
        if target_path == file_path:
    ...
    def _build_main_empty_state(self, items):
        ...
        empty = customtkinter.CTkFrame(self.list_frame.canvas, fg_color="transparent")
        empty_icon = customtkinter.CTkLabel(
            empty, text="", image=self.search_empty_icon
        )
        empty_icon.pack(anchor="w", pady=(0, 10))
        if self.current_mode == "search" and self.last_search_query:
            empty_text = customtkinter.CTkLabel(
                empty,
                text="No sounds found for this search.",
                font=customtkinter.CTkFont(family="Segoe UI", size=18),
            )
            empty_text.pack(anchor="w")
        ...
        action = customtkinter.CTkButton(
            ...
            command=self.reload_current_view,
        ...
        if file_path.exists():
            actions.extend(
                [
                    ...
                    (
                        "Rename",
                        lambda current=file_path: self.rename_downloaded_item(current),
                    ),
                    (
                        "Delete",
                        lambda current=file_path: self.delete_downloaded_item(current),
                    ),
                ]
            )
        else:
            actions.append(
                ("Download", lambda current=item: self.download_item(current))
            )
            actions.append(("Open Downloads Folder", self.open_download_folder))
        actions.append(
            (
                "Copy Path",
                lambda current=file_path: self.copy_path_to_clipboard(current),
            )
        )
    ...
    def render_items(self, items, status_message):
        ...
        visible_items = [
            item
            for item in items
            if not self.hide_downloaded
            or not target_path_for(self.download_dir, item["title"]).exists()
        ]
        if self.hide_downloaded:
            self.result_var.set(
                f"{len(visible_items)} shown • {len(items)} sounds total"
            )
        else:
        ...
        def _worker():
            try:
                ...
                self.app.after(
                    0,
                    self.set_status,
                    f"Search failed on {self.server_base_url}: {exc}",
                )
    ...
    def prefetch_page(self, page_number):
        if (
            page_number <= 0
            or page_number in self.prefetched_pages
            or page_number in self.page_prefetching
        ):
            return
    ...
    def download_all_current(self):
        ...
        if self.current_mode == "page" and all(
            ...
            next_page_number = self.page_no + 1
            self.set_status(
                f"All sounds on page {self.page_no} are already downloaded. Opening page {next_page_number}..."
            )
            self.load_page(next_page_number)
            return
        if self.batch_window is not None and self.batch_window.winfo_exists():
            ...
            return
        next_page_callback = (
            self.download_next_page if self.current_mode == "page" else None
        )
